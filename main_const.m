...
for pp=1:kmax
    ...
    % Calculate a solution considering constant force on time step
    [x, vx] = calc_difx(x0, vx0, fx0, fx0, tstep);
    [y, vy] = calc_dify(y0, vy0, fy0, fy0, tstep);

    % Modify workpiece geometry using cut function
    rw(p,:) = cut(zw(p,:), rw(p,:), zt, rt);

    % Move the tool using feed value and obtained dynamic displacements
    zt = zt + f_h + x - x0;
    rt = rt + y - y0;

    % Check if we have done whole revolution of the workpiece
    if (p==ket)
        pnext = 1;
    else
        pnext = p + 1;
    ...
    [s, h_z, h_r] = geom_thickness(zw(pnext,:), rw(pnext,:), zt, rt);
...
% Graph of the cutting edge and the last machined section of the workpiece
figure(7);
hold on;
...
% Prepare data for machined cylinder visualization
...
% Extend arrays of the surface coordinates to get a closed cylinder
xw(ket + 1,:) = xw(1,:);
yw(ket + 1,:) = yw(1,:);
zw(ket + 1,:) = zw(1,:);
...
% Plot of selected cross section of the workpiece
figure(9)
